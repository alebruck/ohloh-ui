...
class TestFindDuplicateReposWithinProjectTemp < ActiveSupport::TestCase
  ...
  describe 'enlistments' do
    ...
    before do
      create(:enlistment, repository: repository_1)
      create(:enlistment, repository: repository_2)
    ...
    it 'must reassign enlistments from duplicate repo when primary repo lacks enlistment' do
      ...
      duplicate_repo_enlistment = repository_2.enlistments.first

      require_relative './find_duplicate_repos_within_project_temp'

      Enlistment.find_by(id: duplicate_repo_enlistment.id).repository_id.must_equal repository_1.id
    end
